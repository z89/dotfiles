...
- **Never edit `~/.config/i3/config` directly.** i3 config is generated by pywal/wpg from the template at `~/.config/wpg/templates/i3.base`. Edit the template instead, then run `wpg -s <current-wallpaper>` to regenerate the live config.
