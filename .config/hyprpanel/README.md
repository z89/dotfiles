...
The `color-fade` engine in the theme-switch checkout (`$TS`, a separate repo cloned under `~/.config/`) drives smooth HyprPanel color transitions at 30fps. This section documents the full mechanism so it can be rebuilt if anything breaks.
...
- **`config.json`** — HyprPanel settings (bar layout, module config, theme colors). Modified by both manual edits and the theme-switch system in `$TS`.
